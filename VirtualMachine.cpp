	...
	for(uint32_t i = 0; i < myMemPool->MPsize/64; i++)
	{
		if(myMemPool->spaceMap[i] == 0) //nothing in that slot so good
		{
			...
			if(current == numSlots)
			{
				...
				{
					myMemPool->spaceMap[i - j] = memPoolList.size(); //place id at those slots open
...
TVMStatus VMMemoryPoolQuery(TVMMemoryPoolID memory, TVMMemorySizeRef bytesleft)
{
	...
	uint32_t current = 0; //counter to know how many slots are free of memory
    for(uint32_t i = 0; i < myMemPool->MPsize/64; i++)
    {
        if(myMemPool->spaceMap[i] == 0) //if space available, then add it to chunks available
            current++;
    } //go through the space map

    *bytesleft = current * 64; //return bytesleft
...
TVMStatus VMFileRead(int filedescriptor, void *data, int *length)
{
	...
	char *localdata = new char[*length];
  	...
  	uint32_t read = 0; //the number of bytes we have read
...
TVMStatus VMFileWrite(int filedescriptor, void *data, int *length)
{
	...
	char *localdata = new char[*length+1];
  	strcpy(localdata, (char*)data);
  	char *writeloc; //where to write to in location
  	uint32_t written = 0; //the number of bytes we have written
